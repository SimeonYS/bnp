...
class BnpPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bnp.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bnp`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bnp WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bnp (date, title, link, content)"
